...
// -----------------------------------------------------------------
    ...
    public double darTiempoPromedio(){
        int CarrosTotales=0;  int TiempoTotal=0;
        for (Puesto puesto: puestos){
            if(puesto.estaOcupado()){
                CarrosTotales++;
                Carro carro1= puesto.darCarro();
                TiempoTotal+=carro1.darTiempoEnParqueadero(horaActual);
            }
        }
        if(CarrosTotales==0){
            return 0;
        }
        return (double) TiempoTotal/CarrosTotales;
    }
    public boolean hayCarroMasDeOchoHoras() {
        boolean ExcesoCarros= false; 
        int limites= 8;
        int tiempo;
        for (Puesto puesto: puestos){
            if (puesto.estaOcupado()){ 
                Carro carro1= puesto.darCarro(); 
                tiempo = carro1.darTiempoEnParqueadero(horaActual);
                if (tiempo>limites){
                    ExcesoCarros=true;
                }
            }
    ...
    public ArrayList<Carro> darCarrosMasDeTresHorasParqueados( ) {
        ArrayList<Carro> Carro1TRESHORAS= new ArrayList<>();
        int tiempolimite= 3;
        int tiempo;
        for (Puesto puesto: puestos){
            if(puesto.estaOcupado()){
                ...
                if (tiempo>tiempolimite){
                    Carro1TRESHORAS.add(carro1);
                }
            }
        }
        return Carro1TRESHORAS;
    }
    public boolean hayCarrosPlacaIgual() {
        String placa1;
        ArrayList<String> placas= new ArrayList<>();
        for (Puesto puesto: puestos){
            placa1 = puesto.darCarro().darPlaca();
            if (placas.contains(placa1)){
                return true;
            }else{
                System.out.println("diferente placa");
            }
        }
        return false;
    }
    ...
    public boolean hayCarroCon24Horas(){
        ...
        for (Puesto puesto: puestos){
            if(puesto.estaOcupado()){
                ...
                tiempo = carro1.darTiempoEnParqueadero(horaActual);
                if (tiempo>=limite){
                    System.out.println("limite de tiempo superado");
                    return true;
                }
            }
        }
    ...
    public String metodo1( )
    {
        ...
        String mensaje;
        int PB;
        PB = contarCarrosQueComienzanConPlacaPB();
        max_carro = hayCarroCon24Horas();
        mensaje = "carros placa PB: " +PB+
                "\nse supero el tiempo limite?(24 horas)\n";
        if (max_carro){
            mensaje+= "Si";
        }else{
            mensaje+= "No";
